...
class SliderHandle: ObservableObject {
    ...
    func setValue(newValue: Double) {
        currentPercentage.wrappedValue = newValue
    }
}
...
class CustomSlider: ObservableObject {
    ...
    init(start: Double, end: Double, startPosition: Double, endPosition: Double) {
        valueStart = start
        valueEnd = end
        
        highHandleStartPercentage = startPosition;
        lowHandleStartPercentage = endPosition;
    ...
    func setValuye()  {
        lowHandle.currentPercentage.wrappedValue = 30
    }
